...
protected:
    ...
    virtual ~OsrDropTargetWin();

private:
    OsrDragEvents* _callback;                    ///< Event callback handler
    HWND _hWnd;                                  ///< Window handle
    CefRefPtr<CefDragData> _currentDragData;     ///< Current drag data for in-window drag
    CComPtr<IDropTargetHelper> _dropTargetHelper; ///< Drop target helper for drag image display
};
...
private:
    CComPtr<IDragSourceHelper> _dragSourceHelper; ///< Drag source helper for drag image display
};
