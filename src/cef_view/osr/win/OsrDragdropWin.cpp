...
CComPtr<OsrDropTargetWin> OsrDropTargetWin::Create(OsrDragEvents* callback, HWND hWnd) {
    CComPtr<OsrDropTargetWin> target(new OsrDropTargetWin(callback, hWnd));

    // Create IDropTargetHelper to display drag image
    HRESULT hr = CoCreateInstance(CLSID_DragDropHelper, nullptr, CLSCTX_INPROC_SERVER,
                                   IID_IDropTargetHelper,
                                   reinterpret_cast<void**>(&target->_dropTargetHelper));
    if (FAILED(hr)) {
        LOGW << "OsrDropTargetWin::Create: Failed to create IDropTargetHelper, hr=0x"
             << std::hex << hr << std::dec;
    }
...
OsrDropTargetWin::~OsrDropTargetWin() {
...
HRESULT OsrDropTargetWin::DragEnter(IDataObject* dataObject,
    ...
    // Notify IDropTargetHelper first to show drag image
    if (_dropTargetHelper) {
        POINT pt = {cursorPosition.x, cursorPosition.y};
        _dropTargetHelper->DragEnter(_hWnd, dataObject, &pt, *effect);
    }
...
HRESULT OsrDropTargetWin::DragOver(DWORD keyState, POINTL cursorPosition, DWORD* effect) {
    ...
    // Notify IDropTargetHelper to update drag image position
    if (_dropTargetHelper) {
        POINT pt = {cursorPosition.x, cursorPosition.y};
        _dropTargetHelper->DragOver(&pt, *effect);
    }
...
HRESULT OsrDropTargetWin::DragLeave() {
    ...
    // Notify IDropTargetHelper to hide drag image
    if (_dropTargetHelper) {
...
HRESULT OsrDropTargetWin::Drop(IDataObject* dataObject,
    ...
    // Notify IDropTargetHelper before completing drop
    if (_dropTargetHelper) {
        POINT pt = {cursorPosition.x, cursorPosition.y};
        _dropTargetHelper->Drop(dataObject, &pt, *effect);
    }
...
DropSourceWin::~DropSourceWin() {
    // Release IDragSourceHelper
    if (_dragSourceHelper) {
        _dragSourceHelper.Release();
    }
}
...
HBITMAP DropSourceWin::CreateBitmapFromCefImage(CefRefPtr<CefImage> cefImage, float scaleFactor, int& outWidth, int& outHeight) {
    ...
    if (!bitmapData || bitmapData->GetSize() == 0) {
        LOGE << "CreateBitmapFromCefImage: failed to get bitmap data";
        return nullptr;
    }
    ...
    // CEF provides premultiplied alpha, but Windows IDragSourceHelper needs non-premultiplied
    ...
    for (size_t i = 0; i < dataSize; i += 4) {
        ...
        if (a > 0 && a < 255) {
            // Un-premultiply: color = (color_premul * 255 + alpha / 2) / alpha
            // Adding alpha/2 for proper rounding
            buffer[i] = static_cast<unsigned char>(std::min(255, (b * 255 + a / 2) / a));
            buffer[i + 1] = static_cast<unsigned char>(std::min(255, (g * 255 + a / 2) / a));
            buffer[i + 2] = static_cast<unsigned char>(std::min(255, (r * 255 + a / 2) / a));
        }
        // If a == 0, keep RGB as is (fully transparent)
        // If a == 255, no change needed (fully opaque, r/g/b already correct)
    }

    // Create 32-bit BGRA DIB section
    BITMAPINFO bmi = {};
    ...
    if (hBitmap && bits) {
        ...
        LOGE << "CreateBitmapFromCefImage: failed to create DIB section, error=" << GetLastError();
    }
...
void DropSourceWin::SetDragImage(CefRefPtr<CefDragData> dragData, IDataObject* dataObject, float scaleFactor) {
    if (!dragData || !dataObject) {
        LOGE << "SetDragImage: dragData or dataObject is null";
        return;
    }

    // Try to get image from drag data
    if (!dragData->HasImage()) {
        return;  // No image available, skip drag image display
    }

    CefRefPtr<CefImage> cefImage = dragData->GetImage();
    if (!cefImage || cefImage->IsEmpty()) {
        return;  // Invalid image, skip drag image display
    }

    int imageWidth = 0;
    int imageHeight = 0;
    HBITMAP hBitmap = CreateBitmapFromCefImage(cefImage, scaleFactor, imageWidth, imageHeight);
    if (!hBitmap) {
        LOGW << "SetDragImage: Failed to create bitmap from CEF image";
        return;
    }

    CefPoint hotspot = dragData->GetImageHotspot();

    // Use IDragSourceHelper to set drag image
    // This works with IDropTargetHelper on the drop target side to display drag image
    HRESULT hr = CoCreateInstance(CLSID_DragDropHelper, nullptr, CLSCTX_INPROC_SERVER,
                                   IID_IDragSourceHelper, reinterpret_cast<void**>(&_dragSourceHelper));

    if (SUCCEEDED(hr) && _dragSourceHelper) {
        ...
        dragImage.crColorKey = CLR_NONE;  // Use alpha channel for transparency

        hr = _dragSourceHelper->InitializeFromBitmap(&dragImage, dataObject);
        if (FAILED(hr)) {
            LOGW << "SetDragImage: InitializeFromBitmap failed with hr=0x" << std::hex << hr << std::dec;
            _dragSourceHelper.Release();
        }
    } else {
        LOGW << "SetDragImage: Failed to create IDragSourceHelper, hr=0x" << std::hex << hr << std::dec;
    }

    // Clean up bitmap - IDragSourceHelper has copied the data
