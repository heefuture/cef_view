...
void OsrRendererD3D11::render() {
    ...
    // Apply pending resize if any (CEFClient pattern: resize during render)
...
void OsrRendererD3D11::setBounds(int x, int y, int width, int height) {
    ...
    // Input dimensions are already in physical pixels (from SetWindowPos)
    // No need to apply DPI scaling here
    int physicalWidth = width;
    int physicalHeight = height;

    // Time-based debounce: prevent excessive setBounds calls (within 50ms)
    auto now = std::chrono::steady_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - _lastSetBoundsTime);

    if (elapsed.count() < 50 && _hasPendingResize) {
        // Already have a pending resize and this call is too soon
        // Update pending dimensions but don't reset the timestamp
        _pendingWidth = width;
        _pendingHeight = height;
        return;
    }

    // Record pending physical dimensions (defer actual resize to next render)
    _pendingWidth = width;
    _pendingHeight = height;
    _hasPendingResize = true;
    _lastSetBoundsTime = now;
}
...
void OsrRendererD3D11::applyPendingResize() {
    if (!_hasPendingResize) {
        return;
    }

    // Check if dimensions actually changed (built-in debounce like CEFClient)
    if (_pendingWidth == _viewWidth && _pendingHeight == _viewHeight) {
        _hasPendingResize = false;
        return;
    }

    LOGD << "OsrRendererD3D11::applyPendingResize: Executing resize from " << _viewWidth << "x" << _viewHeight
         << " to " << _pendingWidth << "x" << _pendingHeight;
    ...
    // Check for device validity before resizing
    if (!_d3dDevice || !_d3dContext) {
        LOGE << "OsrRendererD3D11::applyPendingResize: Device or context is null, cannot resize";
        return;
    }
    ...
    // Get current swap chain description to preserve format and flags
    DXGI_SWAP_CHAIN_DESC1 desc;
    HRESULT hr = _swapChain->GetDesc1(&desc);
    if (FAILED(hr)) {
        LOGE << "OsrRendererD3D11::applyPendingResize: GetDesc1 FAILED hr=0x" << std::hex << hr;
        return;
    }

    hr = _swapChain->ResizeBuffers(0, _viewWidth, _viewHeight, desc.Format, desc.Flags);
    if (FAILED(hr)) {
        LOGE << "OsrRendererD3D11::applyPendingResize: ResizeBuffers FAILED hr=0x" << std::hex << hr;
        handleDeviceLost();
        return;
    }

    LOGD << "OsrRendererD3D11::applyPendingResize: ResizeBuffers succeeded, recreating RTV";
