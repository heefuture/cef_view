...
protected:
    ...
    /**
     * @brief Apply pending resize operation (called during render)
     * Following CEFClient pattern: resize is deferred until render time
     */
...
protected:
    ...
    // Pending resize state (deferred until next render)
    int _pendingWidth = 0;
    int _pendingHeight = 0;
    bool _hasPendingResize = false;
    std::chrono::steady_clock::time_point _lastSetBoundsTime;
};
