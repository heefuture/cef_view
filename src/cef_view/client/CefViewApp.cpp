...
void CefViewApp::OnBeforeCommandLineProcessing(const CefString& process_type, CefRefPtr<CefCommandLine> command_line)
{
    ...
    // Disable the keychain prompt on macOS.
    ...
    // Allow delegates to add custom command line switches
...
void CefViewApp::OnRegisterCustomSchemes(CefRawPtr<CefSchemeRegistrar> registrar)
{
    // Allow delegates to register custom schemes
    for (auto& weakDelegate : _viewAppDelegates) {
        if (auto delegate = weakDelegate.lock()) {
            delegate->onRegisterCustomSchemes(registrar);
        }
    }
}
