...
public:
    ...
    /**
     * @brief Notify the browser that screen info has changed
     */
    virtual void notifyScreenInfoChanged();

    /**
     * @brief Notify the browser that move or resize has started
     */
...
protected:
    ...
    // Hit test handler for non-client area detection
    virtual LRESULT onHitTest(WPARAM wParam, LPARAM lParam);

    // Mouse message handler, return true if handled
    virtual bool onMouseEvent(UINT uMsg, WPARAM wParam, LPARAM lParam);
